...
import { Application, Comment } from '../types';
...
const AdminDashboard: React.FC = () => {
  ...
  const [reflections, setReflections] = useState<Comment[]>([]);
  const [loading, setLoading] = useState(true);
  const [activeTab, setActiveTab] = useState<'applications' | 'reflections'>('applications');
  ...
  const fetchAllData = async () => {
    ...
    try {
      // Fetch Applications
      const appQ = query(collection(db, 'applications'), orderBy('submittedAt', 'desc'));
      ...
      // Fetch Reflections
      const refQ = query(collection(db, 'reflections'), orderBy('createdAt', 'desc'));
      const refSnapshot = await getDocs(refQ);
      const refs = refSnapshot.docs.map(doc => {
        const data = doc.data();
        return {
          id: doc.id,
          name: data.name,
  ...
  const toggleReflectionApproval = async (refId: string, currentStatus: boolean) => {
    try {
      await updateDoc(doc(db, 'reflections', refId), { approved: !currentStatus });
      setReflections(prev => prev.map(r => r.id === refId ? { ...r, approved: !currentStatus } : r));
    } catch (err) {
      console.error(err);
  ...
  const deleteReflection = async (refId: string) => {
    if (!window.confirm("Are you sure you want to delete this reflection?")) return;
    try {
      await deleteDoc(doc(db, 'reflections', refId));
    ...
    <div className="min-h-screen bg-[#F5F5F5] text-[#141414] font-sans selection:bg-emerald-100">
      ...
      <nav className="fixed top-0 left-0 bottom-0 w-20 bg-white border-r border-stone-200 flex flex-col items-center py-8 z-50">
        ...
        <div className="space-y-8 flex-grow">
           <button 
             ...
             className={`p-3 rounded-xl transition-all shadow-sm ${activeTab === 'applications' ? 'text-emerald-900 bg-emerald-50' : 'text-stone-400 hover:text-emerald-900'}`}
           >
             <Users size={20} />
           </button>
           <button 
             onClick={() => setActiveTab('reflections')}
             className={`p-3 rounded-xl transition-all shadow-sm ${activeTab === 'reflections' ? 'text-emerald-900 bg-emerald-50' : 'text-stone-400 hover:text-emerald-900'}`}
           >
      ...
      <main className="pl-20 min-h-screen">
        ...
        <header className="bg-white border-b border-stone-200 px-12 py-8 flex justify-between items-center sticky top-0 z-40 backdrop-blur-md bg-white/80">
          <div>
            <h1 className="text-2xl font-bold serif italic tracking-tight">
              Scholarship Board <span className="text-emerald-600">/ {activeTab === 'applications' ? 'Applications' : 'Reflections'}</span>
            </h1>
            <p className="font-mono text-[10px] text-stone-400 uppercase tracking-[0.3em] mt-1">
              {activeTab === 'applications' ? 'Reviewing the next generation of Ogbomoso stars' : 'Managing community voices and tributes'}
            </p>
          </div>
        ...
        <div className="p-12 max-w-7xl">
          {activeTab === 'applications' ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {reflections.map((ref) => (
                <div key={ref.id} className="bg-white p-8 rounded-[2rem] border border-stone-200 shadow-sm flex flex-col justify-between">
                  <div>
                    <div className="flex justify-between items-start mb-4">
                      <div>
                        <h3 className="font-bold text-emerald-950">{ref.name}</h3>
                        <p className="text-[10px] text-stone-400 uppercase tracking-widest">{ref.role}</p>
                      </div>
                      <span className={`px-2 py-0.5 rounded-md text-[8px] font-bold uppercase tracking-widest ${ref.approved ? 'bg-emerald-100 text-emerald-700' : 'bg-stone-100 text-stone-400'}`}>
                        {ref.approved ? 'Public' : 'Hidden'}
                      </span>
                    </div>
                    <p className="text-sm text-stone-600 leading-relaxed italic mb-6 line-clamp-4">"{ref.text}"</p>
                  </div>
                  
                  <div className="pt-4 border-t border-stone-50 flex justify-between items-center">
                    <span className="text-[10px] font-mono text-stone-300">{ref.date}</span>
                    <div className="flex space-x-2">
                       <button 
                        onClick={() => toggleReflectionApproval(ref.id, !!(ref as any).approved)}
                        className="p-2 hover:bg-emerald-50 text-stone-400 hover:text-emerald-600 transition-colors rounded-lg"
                        title={ref.approved ? "Hide from site" : "Show on site"}
                       >
                         {ref.approved ? <EyeOff size={16} /> : <Eye size={16} />}
                       </button>
                       <button 
                        onClick={() => deleteReflection(ref.id)}
                        className="p-2 hover:bg-red-50 text-stone-400 hover:text-red-500 transition-colors rounded-lg"
